...
Vue.createApp({
    ...
    computed : {
        ...
        uncoveredCard() {
            let uncoveredCard = [...this.selectCard , ...this.paireCard];
            console.log(uncoveredCard)
            return uncoveredCard;
        }
    },
    methods : {
        handleSerlectCard(card) {
            this.selectCard.push(card);
            if (this.selectCard.length === 2) {
                const [card1, card2] = this.selectCard;
                if(card1.id === card2.id) {
                    this.paireCard.push(card1);
                    this.paireCard.push(card2);
                } else {
        ...
        handleResetGame() {
            const number = this.gameData.selectDeck;
            const pokemonCardList = this.createPokemonByDeck(number);
            const pairpokemonList = this.createPairPokemon(pokemonCardList);
            this.gameData.badSelect = 5;
            this.paireCard = [];
            this.selectCard = [];
        ...
        createPokemonByDeck(deck) {
            ...
            for (let index = 0; index < deck; index++) {
                ...
                const pokemonCard = dataPokemons[number];
